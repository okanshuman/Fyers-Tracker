...
def is_valid_symbol(symbol):
    ...
    return not any(sub in symbol for sub in ["BEES", "ETF", "ALPHA", "MAFANG", "MOREALTY", "MOMOMENTUM"]) and not any(char.isdigit() for char in symbol)
    ...
    try:
        ...
        for holding in holdings_data:
            if is_valid_symbol(holding['symbol']):
                ...
                holding['marketVal'] = round_to_two_decimal(holding.get('marketVal', 0))  # Ensure marketVal is rounded too
